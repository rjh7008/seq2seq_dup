    ...
    for src, tgt, src_len, tgt_len, original_src, original_tgt in trainloader:
        ...
        try:
            ...
            if config.max_split == 0:
                #print(torch.sum(loss).dtype)
                #print(num_total.dtype)
                #loss = torch.sum(loss) / (num_total.float())
                total = (torch.sum(loss[0]) / (num_total.float())) + (loss[1]*0.1)
                #loss.backward()
                total.backward()
            optim.step()
            
            params['report_loss'] += total#loss.data
...
def eval_model(model, datas, params):
    ...
    for i,(src, tgt, src_len, tgt_len, original_src, original_tgt) in enumerate(validloader):
